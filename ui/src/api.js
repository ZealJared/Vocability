export default class Api {
  ...
  request (method, url, data, noUser) {
    ...
    const user = this.user || this.savedUser || null
    if (user !== null) {
      options.headers = {
  ...
  async deleteUser (id) {
    const user = await this.request('GET', `user/${id}/delete`)
    if (user.Deleted) {
      this.router.push('/admin')
    }
  }
  ...
  async getWords () {
    if (this.store.getters.words.length === 0) {
      const { words } = await this.request('GET', 'words')
  ...
  async deleteWord (id) {
    const word = await this.request('GET', `word/${id}/delete`)
    if (word.Deleted) {
  ...
  async getStories () {
    const { stories } = await this.request('GET', 'stories')
    return stories
  }
  ...
  async deleteStory (id) {
    const story = await this.request('GET', `story/${id}/delete`)
    if (story.Deleted) {
      this.router.push('/admin')
    }
  }
  ...
  async getWordLists (storyId) {
    const { wordLists } = await this.request('GET', `story/${storyId}/word_lists`)
    return wordLists
  }
  ...
  async deleteWordList (id) {
    const wordList = await this.request('GET', `word_list/${id}/delete`)
    if (wordList.Deleted) {
      this.router.push('/admin')
    }
  }
  ...
  async getWordListWords (wordListId) {
    const { words } = await this.request('GET', `word_list/${wordListId}/words`)
    return words
  }

  async getWordListWordsIncludeStoryPageId (wordListId) {
    const { words } = await this.request('GET', `word_list/${wordListId}/words/include_story_page_id`)
    return words
  }
  ...
  async getStoryPages (storyId) {
    const { storyPages } = await this.request('GET', `story/${storyId}/story_pages`)
    return storyPages
  }
  ...
  async deleteStoryPage (id) {
    const storyPage = await this.request('GET', `story_page/${id}/delete`)
  ...
  async getStoryPageWords (storyPageId) {
    const { words } = await this.request('GET', `story_page/${storyPageId}/words`)
    return words
  }
  ...
  async getUserTroubleWords (userId) {
    const { userWords } = await this.request('GET', `user/${userId}/trouble_words`)
    return userWords
  }
